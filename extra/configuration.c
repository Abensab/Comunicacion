...
struct ClientCardTag clientCardGenerator(config_t* cfg){
...
struct ClientSoundTag clientSoundGenerator(config_t* cfg){
...
struct ClientSpeekersTag clientSpeekersGenerator (config_t* cfg){
...
int main()
{
    ...
    // Read config Card
    struct ClientCardTag card = clientCardGenerator(&cfg);
    // Print config Card
    printf("%-30s  %d  %d  %d  %d\n", card.pcm_name, card.frame_Rate, card.pcm_buffer_size, card.pcm_period_size, card.buffer);

    // Read config Sound
    struct ClientSoundTag sound = clientSoundGenerator(&cfg);
    ...
    struct ClientSpeekersTag speekers = clientSpeekersGenerator(&cfg);
