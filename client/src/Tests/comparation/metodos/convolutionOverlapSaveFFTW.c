...
void convolutionFFTW(float* x, size_t lx, float *h, size_t lh, float y[])
{
    ...
    for(i=0;i<lx;i++) {
        X[i] = x[i]+0.0f*_Complex_I;
    }

    for(i=0;i<lh;i++) {
        H[i] = h[i]+0.0f*_Complex_I;
    }
...
void convolutionOverlapSaveFFTW(float* x, size_t lx, float localBuffer[], size_t lb, float* h, float y[]) {
    ...
    convolutionFFTW(localBuffer,lb, h, lb, localBuffer);
...
//    printf("Elapsed: %.20f miliseconds => convolution\n", elapsedTime/nTimes );
    ...
    for (i = 0; i < nTimes; ++i) {
        ...
        for (k = 0; k <= (int) iterations + 1; ++k) { // +1 en caso de que se tenga que el resto no de exactamente 0
            ...
            convolutionOverlapSaveFFTW(processBuffer_2, bs, localBuffer, l_localBuffer, tfilter_2, tresult_2);
